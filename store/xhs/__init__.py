...
async def save_creator(user_id: str, creator: Dict):
    ...
    """
    utils.logger.info(f"[store.xhs.save_creator] CALLED with user_id={user_id}")
    utils.logger.info(f"[store.xhs.save_creator] creator keys: {list(creator.keys()) if creator else 'None'}")
    ...
    local_db_item = {
        ...
        'tag_list': json.dumps({tag.get('tagType'): tag.get('name')
                                for tag in (creator.get('tags') or [])}, ensure_ascii=False),  # Tags
        "last_modify_ts": utils.get_current_timestamp(),  # Last modification timestamp (Generated by MediaCrawler, mainly used to record the latest update time of a record in DB storage)
    }
    utils.logger.info(f"[store.xhs.save_creator] creator:{local_db_item}")
    utils.logger.info(f"[store.xhs.save_creator] About to call store_creator()")
    try:
        await XhsStoreFactory.create_store().store_creator(local_db_item)
        utils.logger.info(f"[store.xhs.save_creator] ✓ store_creator() completed successfully")
    except Exception as e:
        utils.logger.error(f"[store.xhs.save_creator] ✗ store_creator() failed: {e}", exc_info=True)
